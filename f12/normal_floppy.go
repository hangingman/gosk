...
// NormalFloppyConfig is the configuration for various properties of
// a new normal floppy formatted block device. Once this configuration is used
// to format a device, it must not be modified.
type NormalFloppyConfig struct {
...
// Formats an fs.BlockDevice with the "normal floppy" format according
// to the given configuration. The "normal floppy" standard means that the
...
func FormatNormalFloppy(device fs.BlockDevice, config *NormalFloppyConfig) error {
	formatter := &normalFloppyFormatter{
		config: config,
		device: device,
	}
...
type normalFloppyFormatter struct {
	config *NormalFloppyConfig
	device fs.BlockDevice
}

func (f *normalFloppyFormatter) format() error {
...
func (f *normalFloppyFormatter) ReservedSectorCount() uint16 {
	if f.config.FATType == FAT32 {
		return 32
	} else {
		return 1
...
func (f *normalFloppyFormatter) SectorsPerCluster() (uint8, error) {
	if f.config.FATType == FAT12 {
...
func (f *normalFloppyFormatter) defaultSectorsPerCluster12() (uint8, error) {
	var result uint8 = 1
...
func (f *normalFloppyFormatter) defaultSectorsPerCluster16() (uint8, error) {
	sectors := f.device.Len() / int64(f.device.SectorSize())
...
func (f *normalFloppyFormatter) defaultSectorsPerCluster32() (uint8, error) {
	sectors := f.device.Len() / int64(f.device.SectorSize())
...
func (f *normalFloppyFormatter) fatCount() uint8 {
	return 2
}

func (f *normalFloppyFormatter) sectorsPerFat(rootEntCount uint16, sectorsPerCluster uint8) uint32 {
